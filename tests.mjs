...
test('opening plus 33 complete bilingual artworks',()=>{assert.equal(works.length,33);assert.equal(works[0].id,'00');assert.equal(works[0].title.zh,'一趟没有终点的远行');assert.equal(works[0].title.it,'Un viaggio senza fine'); for(const w of works){assert.ok(w.body.zh.length);assert.ok(w.body.it.length); assert.ok(w.image);}});
...
test('safe defaults and persisted preferences',()=>{assert.deepEqual(preferences(null),{language:'zh',size:'A+'});assert.deepEqual(preferences('{bad'),{language:'zh',size:'A+'});assert.deepEqual(preferences('{"language":"it","size":"A"}'),{language:'it',size:'A'});});
...
test('mobile narration stays within the download budget',()=>{
  const directory=new URL('./dist/audio/',import.meta.url);
  const total=readdirSync(directory).filter(name=>name.endsWith('.mp3')).reduce((sum,name)=>sum+statSync(new URL(name,directory)).size,0);
  assert.ok(total<=43_000_000);
});
